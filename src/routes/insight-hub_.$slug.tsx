...
export const Route = createFileRoute("/insight-hub_/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: `${insight.title} — Insight Hub | KRISNApedia` },
        { name: "description", content: insight.desc },
        { property: "og:title", content: insight.title },
        { property: "og:description", content: insight.desc },
        { property: "og:image", content: insight.cover },
        { name: "twitter:image", content: insight.cover },
      ],
    };
  },
  notFoundComponent: () => (
...
function ContentViewer({
  ...
  insight: ReturnType<typeof getInsightBySlug> & {};
}) {
  if (!insight) return null;

  if (insight.viewer === "video") {
    return <VideoViewer src={insight.videoSrc ?? ""} poster={insight.cover} />;
  }

  if (insight.viewer === "web") {
    return (
      <article className="rounded-2xl border border-border bg-card p-8 shadow-[var(--shadow-soft)] md:p-12">
        ...
        <div className="prose prose-neutral max-w-none">
          ...
          <ul className="mt-3 space-y-2 text-muted-foreground">
            <li>• Konsolidasi sumber dokumentasi yang tersebar.</li>
            <li>• Penerapan governance yang ringan tetapi konsisten.</li>
            <li>• Otomatisasi sinkronisasi antar tools.</li>
          </ul>
        </div>
      </article>
    );
  ...
  if (insight.viewer === "image") {
    return (
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-soft)]">
        <img src={insight.cover} alt={insight.title} className="w-full" />
      </div>
    );
  }

  // PDF or Slides — emulate a document viewer using cover as page preview
  return <DocumentViewer insight={insight} />;
}

function DocumentViewer({
  insight,
}: {
  insight: NonNullable<ReturnType<typeof getInsightBySlug>>;
}) {
  const totalPages = insight.pages ?? 12;
  const [page, setPage] = useState(1);
  const isPdf = insight.viewer === "pdf";
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-foreground shadow-[var(--shadow-soft)]">
      ...
      <div className="flex items-center justify-between border-b border-background/10 bg-foreground px-4 py-3 text-background">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="text-background/70">{insight.title}</span>
        </div>
        ...
        <div className="relative aspect-[4/5] w-full max-w-2xl overflow-hidden rounded-md bg-card shadow-2xl md:aspect-[4/3]">
          ...
          <div className="absolute inset-0 flex flex-col p-8 md:p-12">
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em] text-primary">
              {insight.category} · Halaman {page}
            </span>
            <h3 className="mt-3 font-display text-2xl font-bold text-foreground md:text-3xl">
              {page === 1 ? insight.title : `Bagian ${page}`}
            </h3>
            <p className="mt-3 max-w-md text-sm leading-relaxed text-muted-foreground md:text-base">
              {insight.longDesc.slice(0, 220)}…
            </p>
            <div className="mt-auto flex items-center justify-between text-[11px] text-muted-foreground">
              <span>{insight.author}</span>
              <span>KRISNApedia · {insight.date}</span>
            </div>
          </div>
        </div>
      </div>
